...
final class CommonTests: XCTestCase {
  ...
  func test_getItemList_success_case() async {
    ...
    /// - Note: Repo와 User에 대한 Mocke을 Common에 설정
    let responeMock: GithubEntity.Common = .init(
      repoList: [repoMock],
      userList: [userMock])
...
extension CommonTests {
  struct SUT {
    ...
    init(state: CommonReducer.State = .init()) {
    ...
    let store: TestStore<CommonReducer.State, CommonReducer.Action>
  }
