...
final class RepoDetailTests: XCTestCase {
  ...
  func test_binding() async {
    ...
    await sut.store.send(.set(\.fetchIsMe, .init(isLoading: false, value: true))) { state in
  ...
  func test_getIsMe_success_case2() async {
    ...
    /// - Note: Me 리스트에 해당 아이템이 있음 => 좋아요인 상태
    sut.container.githubMeUseCaseFake.reset(store: .init(repoList: [responseMock]))
  ...
  func test_updateIsMe_success_case1() async {
    ...
    /// - Note: 현재 저장된 Repo가 없는 경우 = UnMe인 상태
    sut.container.githubMeUseCaseFake.reset()
    ...
    await sut.store.receive(\.fetchIsMe) { state in
      state.fetchIsMe.isLoading = false
      /// - Note: 현재 아이템이 UnMe 상태인데 위에서 updateIsMe를 수행했으므로 Me인 상태(true)로 변경
      state.fetchIsMe.value = true
    }
  }
  ...
  func test_updateIsMe_success_case2() async {
    ...
    /// - Note: 현재 Repo가 좋아요 리스트에 있는 상태 = Me
    sut.container.githubMeUseCaseFake.reset(store: .init(repoList: [responseMock]))
    ...
    await sut.store.receive(\.fetchIsMe) { state in
      state.fetchIsMe.isLoading = false
      /// - Note: 현재 아이템이 Me 상태인데 위에서 updateIsMe를 수행했으므로 UnMe인 상태(false) 로 변경
      state.fetchIsMe.value = false
    }
  }
